...
class RestrictClassificationTokensProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores):
        ...
        if torch.lt(self.gen_token, self.step_cls_tokens.shape[0]-1):
            ...
            eos_mask_not_offensive = torch.logical_not(
                torch.eq(input_ids[:,-self.gen_token], self.cls_tokens[0])
            ).type(torch.uint8).reshape(scores.shape[0], 1)  #(B, 1)

            seps_mask = torch.logical_and(
                torch.eq(torch.sum(torch.eq(input_ids, self.sep_token_id), dim=-1), 4),
                torch.logical_not(
                    torch.logical_or(
                        torch.eq(input_ids[:,-1], self.cls_tokens[-2]), torch.eq(input_ids[:,-1], self.cls_tokens[-1])
                    )
                )
            ).type(torch.uint8).reshape(scores.shape[0], 1)

            eos_mask_end = torch.logical_or(
                torch.eq(input_ids[:,-1], self.cls_tokens[-2]), torch.eq(input_ids[:,-1], self.cls_tokens[-1])
            ).type(torch.uint8).reshape(scores.shape[0], 1)
            
            eos_mask = self._restrict_tokens(self.eos_token_id, scores)
            eos_mask = torch.mul(torch.add(eos_mask_not_offensive, eos_mask_end), eos_mask)
